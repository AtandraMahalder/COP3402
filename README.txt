...
To run the program, we need a unix environment. Before compilation ensure that driver.c, lex.c, parser.c and compiler.h
are in the same directory. To compile open the terminal, navigate to the directory containing driver.c and
enter the command: gcc driver.c lex.c parser.c -lm.
OR, if there is an object file for scanner, then gcc driver.c lex.o parser.c
...
and then get its syntax checked by the parser
