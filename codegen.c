...
// figure out how to implement recursive prodecures
void procdecl()
{
	if (lex[token].type == procsym)
	{
		...
		// when you have a call, your lex level goes up
		for (int i = curr_code_index; i < code_index; ++i) // bugs here
		{
			if (code[i].opcode == 5 && code[i].l == lexlevel && code[i].m == table[this_sym_index].val)
			{
				lexlevel++;
